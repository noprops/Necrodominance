...
def save_results_to_csv(filename: str, results: list[dict]) -> None:
    """
    結果をCSVファイルに保存する
    ...
    important_fields = [
        ...
        'cast_necro_count', 'cast_necro_rate', 'win_after_necro_rate'
    ]
    fieldnames = [field for field in important_fields if field in all_keys]
    # 残りのフィールドを追加
    fieldnames.extend(sorted(key for key in all_keys if key not in important_fields))
